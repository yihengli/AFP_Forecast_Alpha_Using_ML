...
TICKERS_FILE = 'ticker_snp500.csv'
LOG_FILE = 'log.txt'
...
if __name__ == '__main__':
    ...
    with open(os.path.join(CURRENT_LOC, LOG_FILE), 'w') as f:
        f.write("{} Tickers are found by checking at "
                "the first half:".format(len(renamed_then_work_symbols)))
        f.write(", ".join(renamed_then_work_symbols))

        f.write("---" * 10)

        f.write("{} Tickers are not found on Yahoo".format(
            len(failed_symbols)))
